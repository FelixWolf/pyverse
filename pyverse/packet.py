...
class packet:
    ...
    def __init__(self, data=None, message=None, mid = 0, sequence = 0, zero_coded = 0, reliable = 0, resent = 0, ack = 0, acks = []):
        if data:
            ...
            if self.zero_coded:
                self.bytes = zerocode.decode(data[6+self.extra_bytes:])
            else:
                self.bytes = data[6+self.extra_bytes:]
            offset = 1
    ...
    def __bytes__(self):
        ...
        return struct.pack(">BiB", self.flags, self.sequence, len(self.extra)) + self.extra + result + body + acks